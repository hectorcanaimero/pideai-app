...
import { Camera, Trash2, Info } from "lucide-react-native";
...
export default function ProductDetailScreen() {
  ...
  useEffect(() => {
    if (product) {
      ...
      setDisplayOrder(product.display_order != null ? String(product.display_order) : "");
      setIsAvailable(product.is_available ?? true);
      setIsFeatured(product.is_featured ?? false);
      setAdminOnly(product.admin_only ?? false);
      setTrackStock(product.track_stock);
      setStockQuantity(product.stock_quantity != null ? String(product.stock_quantity) : "");
      setStockMinimum(product.stock_minimum != null ? String(product.stock_minimum) : "0");
    }
  }, [product]);
    ...
    <ScrollView className="flex-1 bg-elegant-dark" contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      ...
      >
        {saving ? (
          ...
          <Text className="text-text-inverted font-sans-bold text-lg">Guardar Cambios</Text>
        )}
      </TouchableOpacity>

      {/* Delete button */}
      <TouchableOpacity
        className="py-3 rounded-xl items-center flex-row justify-center gap-2 border border-red-500/30 mt-3"
        onPress={handleDelete}
        activeOpacity={0.7}
      >
        <Trash2 size={16} color="#EF4444" />
        <Text className="text-red-400 font-sans-medium text-base">Eliminar producto</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
